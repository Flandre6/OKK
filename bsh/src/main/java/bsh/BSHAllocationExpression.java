...
public class BSHAllocationExpression extends SimpleNode
{
        ...
        throws EvalError
    {
        ...
        try {
            ...
            System.err.println("[OKK-Bsh] toObject failed for " + nameNode.text + ": " + e.getMessage());
            e.printStackTrace();
        }
        ...
        if ( obj instanceof ClassIdentifier ) {
            ...
            // Fallback resolution directly via namespace
            try {
                type = callstack.top().getClass(nameNode.text);
            } catch (Throwable e) {
                System.err.println("[OKK-Bsh] getClass fallback failed: " + e.getMessage());
                e.printStackTrace();
            }
            if (type == null) {
                try {
                    type = interpreter.getClassManager().classForName(nameNode.text);
                } catch (Throwable e) {
                    System.err.println("[OKK-Bsh] classForName fallback failed: " + e.getMessage());
                    e.printStackTrace();
                }
            }
            if (type == null) {
