        ...
        throws EvalError
    {
        ...
        if ( node instanceof BSHPrimitiveType )
            ...
            baseType = ((BSHAmbiguousName)node).toClass(
                ...
                // Check if it's a nested class name like XC_MethodHook.MethodHookParam or de.robv.android.xposed.XC_MethodHook.MethodHookParam
                String text = node.getText().trim();
                Class<?> nestedClass = null;
                if (text.indexOf('.') > 0 && callstack != null) {
                    ...
                    for ( int outer = parts.length - 1; outer >= 1; outer-- ) {
                        ...
                        Class<?> clas = null;
                        try {
                            clas = callstack.top().getClass( outerName.toString() );
                        ...
                        if ( clas != null ) {
                            ...
                            for ( int i = outer; i < parts.length; i++ )
                                binary.append('$').append(parts[i]);
                            try {
                                nestedClass = callstack.top().getClass( binary.toString() );
                                if (nestedClass != null) break;
                            } catch ( Throwable ignore ) { }
                        }
                    }
                }
                if (nestedClass != null) {
                    baseType = nestedClass;
                } else if (node.getText().trim().length() == 1
